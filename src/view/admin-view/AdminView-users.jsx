...
import { getAllUsers, updateUserByID, deleteUserById } from '../../redux/actions'
...
const AdminViewUsers = ({ searchQuery }) => {
  ...
  const handleSave = (updatedUser) => {
    dispatch(updateUserByID(updatedUser, token)).then(() => {
      setShowModal(false)
      dispatch(getAllUsers({ pagination: 9999 }, token)) // Refrescar la lista de usuarios después de la actualización
    })
  }
  ...
  const handleDelete = (userId) => {
    if (window.confirm('¿Estás seguro de que deseas eliminar este usuario?')) {
      dispatch(deleteUserById(userId, token)).then(() => {
        dispatch(getAllUsers({ pagination: 9999 }, token)) // Refrescar la lista de usuarios después de la eliminación
      })
    }
  }
